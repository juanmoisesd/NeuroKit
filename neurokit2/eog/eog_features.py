...
def _eog_features_delineate(eog_cleaned, candidates, sampling_rate=1000):
    ...
    for i in epochs:
        ...
        if len(max_frame) > 1:
            ...
            max_frame = int(np.ravel(max_frame)[0])
        ...
        max_position = int(np.where(crossings_idx == max_frame)[0][0])
        ...
        if (max_position - 1) >= 0:  # crosses zero point
            ...
            leftzero = int(np.ravel(leftzero)[0])
        ...
        if (max_position + 1) < len(crossings_idx):  # crosses zero point
            ...
            rightzero = int(np.ravel(rightzero)[0])
